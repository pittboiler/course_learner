...
- **Sideways:** the same folding rule governs stroboscopy, the wagon-wheel effect in film, and moiré patterns in a printed halftone — all of them spatial or temporal undersampling of the same mathematical kind. In [`computational-physics`](../../computational-physics/syllabus.md) it reappears as the reason a time step that is too coarse produces spurious low-frequency oscillation.
