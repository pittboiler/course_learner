...
**P3** Expand around $q^*$ with step $h$: $\pi(q^* + h) \approx \pi(q^*) + \pi'(q^*)\,h + \tfrac{1}{2}\pi''(q^*)\,h^2 = \pi(q^*) + 0 - h^2$. Lost profit $\approx h^2$ dollars. The linear term is dead at an optimum — that's what being an optimum *means* — so small errors cost only second-order money: misjudging optimal output by $h$ costs $\sim h^2$, e.g. a 1-unit mistake costs about a dollar, a 0.1-unit mistake costs a penny. Flat-topped hills forgive small missteps. Lesson 1.4 turns this expansion into the second-derivative test, and Boss problem 1 asks you to interpret $\pi''$ exactly this way.
