...
- **Sideways (quantum mechanics):** invariant subspaces are the mathematical skeleton of *degenerate multiplets* — a symmetry group acting on a Hamiltonian's eigenspaces breaks the state space into irreducible blocks, and the block labels are the "good quantum numbers." Splitting a rep into atoms is literally reading off the allowed energy levels; see `](../../quantum-mechanics/syllabus.md)`.
- **Sideways (linear algebra):** block-triangular vs block-diagonal is the same simultaneous-triangularization/diagonalization story from `](../../linalg-refresher/syllabus.md)`, now applied to a *whole family* of commuting-with-the-group matrices at once rather than a single operator.
