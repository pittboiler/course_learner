...
- **Sideways (quantum mechanics):** decomposing a rep is decomposing a state space into **symmetry multiplets** — the isotypic components $V^{(i)}$ are the degenerate energy sectors, the multiplicities $m_i$ count how many multiplets of each symmetry type occur, and the projectors $P_i$ are the symmetry-adapted projection operators that build good basis states; see [`quantum-mechanics`](../../quantum-mechanics/syllabus.md).
