...
- **Sideways — `](../../econometrics/syllabus.md)`:** moment matching is informal method-of-moments; GMM formalizes "choose parameters so model moments equal sample moments," and the AR(1) shock fit is bread-and-butter time-series estimation. DSGE estimation (likelihood/Bayesian) is the successor to calibration.
- **Sideways — `](../../dynamical-systems/syllabus.md)`:** log-linearization is linearizing a nonlinear map about a fixed point; the eigenvalues of the resulting linear system (inside vs. outside the unit circle) decide stability exactly as in continuous-time phase-plane analysis, here in discrete time.
