...
> Engineering · Tier 2 · ~19 lessons · Prereqs: [condensed-matter](../condensed-matter/syllabus.md), [electronics](../electronics/syllabus.md) · Roadmap id: `semiconductor-devices`

## Goal

Learn how the band theory of `condensed-matter` becomes the working devices of `electronics` — how a piece of doped silicon rectifies, amplifies, switches, emits light, and turns sunlight into current. You will build the transport toolkit (drift, diffusion, generation–recombination, the continuity equations), then use it to derive the p–n junction from its electrostatics all the way to its I–V and capacitance, and go on to the bipolar transistor, the MOS capacitor and MOSFET (with the scaling story that drives Moore's law), and the optoelectronic devices — LED, laser, solar cell, detector. Deliberately skipped: full process/fabrication engineering (we take only a taste at the end) and compound-semiconductor exotica beyond the heterojunction idea. This is a tier-2 course — it assumes you can already read a band diagram, use Fermi–Dirac statistics, and locate a Fermi level. **It therefore defers, rather than repeats, its prerequisites:** carrier statistics and the law of mass action, dopant ionization, the Fermi level's march across temperature, and drift mobility are all taught in [`condensed-matter` Module 4](../condensed-matter/syllabus.md) and are *cited* here, not re-derived; the circuit-level views of the diode, BJT and MOSFET belong to [`electronics`](../electronics/syllabus.md); and the physics of stimulated emission, gain and optical cavities belongs to [`photonics-quantum-optics`](../photonics-quantum-optics/syllabus.md). What this course adds is the *device* layer those courses stop short of: transport out of equilibrium, junction electrostatics solved properly, and the derivations behind every device parameter you have so far been handed.
...
|---|---|---|---|
| 1.1 | Carriers, doping and the Fermi level, reloaded | Reload just enough equilibrium bookkeeping to run a device | $n_i$, $N_c,N_v$, mass action $np=n_i^2$, $n\approx N_d$, minority $=n_i^2/N_d$, $E_F$ placement, freeze-out/extrinsic/intrinsic *(cites [`condensed-matter` 4.1–4.3](../condensed-matter/syllabus.md); this is a lookup pass, not a derivation)* |
| 1.2 | Drift, diffusion and the Einstein relation | Add the transport mechanism equilibrium physics leaves out | diffusion current, Fick's law, $D=(k_BT/q)\mu$, quasi-Fermi levels, total drift+diffusion current, velocity saturation and mobility vs doping *(cites [`condensed-matter` 4.4](../condensed-matter/syllabus.md) for $\mu=e\tau/m^*$ and $\sigma$)* |
| 1.3 | Generation and recombination | Track how carriers are born and die out of equilibrium | thermal equilibrium balance, low-level injection, minority-carrier lifetime $\tau$, SRH traps, Auger and radiative recombination |
| 1.4 | The continuity equations | Assemble conservation into the master transport equation | continuity equation, minority-carrier diffusion equation, diffusion length $L=\sqrt{D\tau}$, steady-state profiles |
...
| # | Lesson | Goal (one line) | Key concepts |
|---|---|---|---|
| 2.1 | Junction electrostatics: depletion, potential, width and field | Solve Poisson's equation across the space-charge region and read off the geometry | depletion approximation, Poisson's equation, $x_nN_d=x_pN_a$, built-in potential $V_{bi}$, depletion width $W(V)$, peak field $\mathcal{E}_{max}$, one-sided junction, band bending under bias *([`condensed-matter` 4.5](../condensed-matter/syllabus.md) states $V_{bi}$ and $W$; here they are derived, and extended to arbitrary bias)* |
| 2.2 | The ideal-diode equation | Derive Shockley's $I$–$V$ from injected minority carriers | law of the junction, minority injection, ideal-diode equation $I=I_0(e^{qV/k_BT}-1)$, saturation current $I_0$ |
| 2.3 | Junction and diffusion capacitance | Read the two ways a diode stores charge | depletion (junction) capacitance $C_j\propto1/\sqrt{V_{bi}-V}$, $1/C^2$ doping plot, stored charge, diffusion capacitance, transit time |
| 2.4 | Reverse breakdown | Explain the sharp reverse current and estimate its voltage | critical field, avalanche multiplication, impact ionization, Zener tunneling, breakdown voltage vs doping |
| 2.5 | Metal–semiconductor and heterojunctions | Meet the Schottky barrier and the engineered band offset (a taste) | Schottky barrier $\phi_B$, ohmic vs rectifying contact, band offset, heterojunction, band-gap engineering |
